...
bool SwiftLanguageRuntime::GetDynamicTypeAndAddress_Protocol(
  ...
  if (in_value.GetValueType() == eValueTypeConstResult) {
    ...
    // Horrible hack -- we have a locally materialized value, so let
    // our MemoryReader know where it is so that RemoteAST can read
    // from it. Also, there's something going on with Error values
    // I don't quite understand.
    is_error = GetSwiftType(protocol_type)
        ->getExistentialLayout().isErrorExistential();
    if (is_error) {
      existential_address = in_value.GetStaticValue()
        ->GetChildAtIndex(0, true)->GetValueAsUnsigned(0);
    } else {
      existential_address = in_value.GetValue().GetScalar().ULongLong();
    }

    use_local_buffer = !is_error;
  } else {
  ...
  if (use_local_buffer)
    PushLocalBuffer(existential_address, in_value.GetByteSize() * 100);

  swift::remote::RemoteAddress remote_existential(existential_address);
  auto result = [&]() -> swift::remoteAST::Result<swift::remoteAST::OpenedExistential> {
    // This is a funny case where lldb hands us the address of the Error
    // container, not the address of the Error value.
    if (is_error) {
      if (log)
        log->Printf("looking at an Error container");
      return remote_ast.getDynamicTypeAndAddressForError(
        remote_existential);
    } else {
      return remote_ast.getDynamicTypeAndAddressForExistential(
